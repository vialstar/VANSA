...
class AdvMixTrainer(object):       
    ...
    def train_one_step(self, data):  
        ...
        # 使用多个生成器生成负样本
        generators = [self.generator1, self.generator2] 
        all_scores = []
        
        for generator in generators:
            batch_gen_hv = generator(batch_hs, 1)
            batch_gen_tv = generator(batch_ts, 1)
            batch_gen_ht = generator(batch_hs, 2)
            batch_gen_tt = generator(batch_ts, 2)
            scores, _ = self.model.model.get_fake_score(  # 计算负样本的得分
                batch_h=batch_h_gen,
                batch_r=batch_r,
                batch_t=batch_t_gen,
                mode=data['mode'],
        ...
        # 集成负样本得分
        integrated_scores = [torch.mean(torch.stack([scores[i] for scores in all_scores]), dim=0) for i in range(len(all_scores[0]))]
        ...
        all_scores_g.append(scores)
        # 集成负样本得分
        integrated_scores_g = [torch.mean(torch.stack([scores[i] for scores in all_scores_g]), dim=0) for i in range(len(all_scores_g[0]))]
        
        loss_g = 0.0  
        for score in integrated_scores_g:
    ...
    def set_alpha(self, alpha): #设置模型的学习率 alpha。
        self.alpha = alpha
